...
//    private DayViewAdapter dayViewAdapter = new DefaultDayViewAdapter();
    ...
    public CalendarPickerView(Context context, AttributeSet attrs) {
        ...
        int bg = a.getColor(R.styleable.CalendarPickerView_android_background, res.getColor(R.color.white));
        dividerColor = a.getColor(R.styleable.CalendarPickerView_tsquare_dividerColor, res.getColor(R.color.white));
        ...
        titleTextStyle = a.getResourceId(R.styleable.CalendarPickerView_tsquare_titleTextStyle, R.style.CalendarTitle);
        displayHeader = a.getBoolean(R.styleable.CalendarPickerView_tsquare_displayHeader, false);
        headerTextColor = a.getColor(R.styleable.CalendarPickerView_tsquare_headerTextColor, res.getColor(R.color.text_minor_dark));
        // 设置每个月的头部是否显示星期
        displayDayNamesHeaderRow = a.getBoolean(R.styleable.CalendarPickerView_tsquare_displayDayNamesHeaderRow, false);
        displayAlwaysDigitNumbers = a.getBoolean(R.styleable.CalendarPickerView_tsquare_displayAlwaysDigitNumbers, false);
        a.recycle();
    ...
    public FluentInitializer init(Date minDate, Date maxDate, TimeZone timeZone, Locale locale) {
        ...
        while ((monthCounter.get(Calendar.MONTH) <= maxMonth // Up to, including the month.
            ...
            MonthDescriptor month = new MonthDescriptor(monthCounter.get(Calendar.MONTH)
                    , monthCounter.get(Calendar.YEAR), date, formatMonthDate(date));
    ...
    public class FluentInitializer {
        ...
        public FluentInitializer displayOnly(boolean can_select) {
            displayOnly = can_select;
            return this;
        }
    ...
    List<List<MonthCellDescriptor>> getMonthCells(MonthDescriptor month, Calendar startCal) {
        ...
        while ((cal.get(Calendar.MONTH) < month.getMonth() + 1 || cal.get(Calendar.YEAR) < month.getYear())
            ...
            for (int c = 0; c < 7; c++) {
                ...
                boolean isCurrentMonth = cal.get(Calendar.MONTH) == month.getMonth();
                ...
                int dayFlag = getDayFlag(cal, today);
                ...
                weekCells.add(new MonthCellDescriptor(date, isCurrentMonth, isSelectable, isSelected
                        , dayFlag ,isHighlighted, value, rangeState));
    ...
    private static int getDayFlag(Calendar cal, Calendar today) {
        if(sameDate(cal, today)){
            return 1;
        }else{
